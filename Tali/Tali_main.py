from Visualization.Tali import Read_KL_Output_File,maps_manager,create_index_file,state_reader,ParseInput,Create_Forward_Index,Creare_Backwards_Index,Create_Vectors_Index
import os
...
def start_project(dataset_path):
    ...
    path = str(path.absolute()) + "\\DataSets" + str(dataset_path)

    # path = "../DataSets"+dataset_path
    file = Read_KL_Output_File.Read_file(KLOutput_path=path+"\\KLOutput")
    file.set_tirps_names(path=path)
    ...
    #                                           forward_index_path=path+"\\forward_index_file.txt")
    # Creare_Backwards_Index.Create_Backwards_Index(backwards_tree=backwards_tree,
    #                                               backwards_index_path=path+"\\backwards_index_file.txt")
    Create_Vectors_Index.Create_Vectors_Index(relations_vectors=relations_vectors,vectors_index_path=path+"\\relations_index_file.txt")
