...
def create_symbol_TIRPs_index(symbol_TIRPs={}):
    ...
    path = os.path.join(f"C:\\Users\\talim\\Desktop\\final_project_server\\karmalegoweb\\src\\Visualization\\Tali\\datasets\\{SYMBOL_TIRPS}")
...
def create_TIRPs_json_index(visualization_path, tirps_json={}):
    path = os.path.join(f"C:\\Users\\talim\\Desktop\\final_project_server\\karmalegoweb\\src\\Visualization\\Tali\\datasets\\{TIRPS_JSON}")
    # path = os.path.join(visualization_path, "Tali", TIRPS_JSON)
    if os.path.exists(path):
        return deserialize_tirps_json(visualization_path=path)
    else:
        # if tirps_json == {}:
...
def create_symbols_names_index(symbols_to_names={}):
    ...
    path = os.path.join(f"C:\\Users\\talim\\Desktop\\final_project_server\\karmalegoweb\\src\\Visualization\\Tali\\datasets\\{SYMBOLS_NAMES_JSON}")
    if os.path.exists(path):
        return deserialize_symbols_to_names(visualization_path=path)
    else:
        if symbols_to_names == {}:
...
def create_properties_index(visualization_path, props={}):
    path = os.path.join(f"C:\\Users\\talim\\Desktop\\final_project_server\\karmalegoweb\\src\\Visualization\\Tali\\datasets\\{PROPERTIES}")
    final_json = {}
    for prop in props:
        final_json[prop] = []
